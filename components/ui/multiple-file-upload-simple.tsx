...
export default function MultipleFileUpload({ 
  ...
  const handleFiles = async (files: FileList) => {
    ...
    try {
      console.log('Starting upload of', validFiles.length, 'files using placeholder URLs')
      
      // For now, let's use placeholder images to avoid Firebase size limits
      ...
      const uploadPromises = validFiles.map(async (file, index) => {
        ...
        // Generate a more reliable placeholder URL
        const fileName = file.name.replace(/\.[^/.]+$/, "") // Remove extension
        const placeholderUrl = `https://picsum.photos/400/300?random=${Date.now()}_${index}`
      ...
      >
        {uploading ? (
          <>
            ...
            <p className="text-primary font-medium mb-1">Processing images...</p>

...
import { uploadImageToHosting } from "@/lib/simple-image-upload"
...
export default function MultipleFileUpload({ 
  ...
  const handleFiles = async (files: FileList) => {
    ...
    try {
      console.log('Starting upload of', validFiles.length, 'files using simple upload service')
      
      const uploadPromises = validFiles.map(async (file, index) => {
        console.log(`Processing file ${index + 1}/${validFiles.length}:`, file.name)
        
        try {
          const result = await uploadImageToHosting(file)
